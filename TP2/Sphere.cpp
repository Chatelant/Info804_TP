...
rt::Sphere::rayIntersection(const Ray &ray, Point3 &p) {
    // Polynome
    Vector3 ap = Vector3(ray.origin[0] - this->center[0],
                         ray.origin[1] - this->center[1],
                         ray.origin[2] - this->center[2]);

    double dist = ap.dot(ap) - pow(ap.dot(ray.direction), 2.0);

    // Si distance² > r² alors pas d'intersection
    if (dist > pow(this->radius, 2.0)) {
        return 1.0f;
    }

    double delta = 4 * pow(ap.dot(ray.direction), 2.0) - 4 * (ap.dot(ap) - pow(radius, 2.0));
    double t1 = (-2 * ap.dot(ray.direction) - sqrt(delta)) / 2;
    double t2 = (-2 * ap.dot(ray.direction) + sqrt(delta)) / 2;

    if (delta < 0) // intersection vide
    {
        return 1.0f;
    }

    if (0 < t1 && t1 < t2) {
        p[0] = ray.origin[0] + t1 * ray.direction[0];
        p[1] = ray.origin[1] + t1 * ray.direction[1];
        p[2] = ray.origin[2] + t1 * ray.direction[2];
    } else if (t1 < 0 && 0 < t2) {
        p[0] = ray.origin[0] + t2 * ray.direction[0];
        p[1] = ray.origin[1] + t2 * ray.direction[1];
        p[2] = ray.origin[2] + t2 * ray.direction[2];
    } else if (t1 < t2 && t2 < 0) {
        return 1.0f;
    }
    return -1.0f;
